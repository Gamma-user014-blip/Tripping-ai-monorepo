...
const getOrCreateSessionId = (sessionIdRef: React.MutableRefObject<string>): string => {
...
const useChat = ({ onTripsLoaded, onSearchStart }: UseChatOptions = {}): UseChatReturn => {
  ...
  useEffect(() => {
    ...
    const storedMessages = sessionStorage.getItem(`${CHAT_MESSAGES_KEY_PREFIX}${sessionId}`)
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(`${CHAT_MESSAGES_KEY_PREFIX}${sessionId}`, JSON.stringify(messages))
  }, [messages])
  ...
  const sendMessage = useCallback(
    async (text: string): Promise<void> => {
      ...
      try {
        ...
        if (response.status === ChatResponseStatus.COMPLETE && response.searchId) {
