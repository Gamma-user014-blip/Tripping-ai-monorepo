...
const ResultsPage: React.FC = (): JSX.Element => {
  ...
  const dates = Array.isArray(query.dates)
    ? query.dates[0]
    : (query.dates as string | undefined);
  const adults = query.adults
    ? Number(Array.isArray(query.adults) ? query.adults[0] : query.adults)
    : undefined;
  const children = query.children
    ? Number(Array.isArray(query.children) ? query.children[0] : query.children)
    : undefined;
