...
import { SearchPollRequest, SearchPollResponse, SearchStatus, Trip } from '../../../shared/types'
...
const pollForSearchResults = (searchId: string, options: PollOptions): StopPolling => {
  ...
  const poll = async (): Promise<void> => {
    ...
    try {
      const response = await apiClient.post<SearchPollResponse>('/api/search/poll', {
        searchId,
