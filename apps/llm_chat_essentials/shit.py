...
# Server-side chat history (Perplexity/OpenAI Chat Completions format)
conversations: Dict[str, List[Dict[str, str]]] = {}


def get_or_create_conversation(conversation_id: str) -> List[Dict[str, str]]:
...
def call_llm(
    ...
    *,
    ...
    messages = get_or_create_conversation(conversation_id)

    # Add system message once per conversation
    ...
    answer = getattr(getattr(completion.choices[0], "message", None), "content", "") or ""
...
if __name__ == "__main__":
    ...
    state = update_trip_yaml_state(
        raw_user_message="2 adults. I want a beach trip, very foodie, sometime in March.",
        current_yaml_state=state,
        conversation_id="alice-trip",
    )
    print("\n=== YAML STATE ===\n", state)

    missing_resp = get_single_missing_question(state, conversation_id="alice-trip-missing")
    print("\n=== FIRST MISSING ===\n", missing_resp)
    ...
    missing_resp = get_single_missing_question(state, conversation_id="alice-trip-missing")
    print("\n=== NEXT MISSING AFTER UPDATE ===\n", missing_resp)

    state = update_trip_yaml_state(
        raw_user_message="I want to change my trip to 3 adults, and the dates to April 5 to April 12.",
        current_yaml_state=state,
        conversation_id="alice-trip",
    )
    print("\n=== UPDATED YAML STATE 2 ===\n", state)

    missing_resp = get_single_missing_question(state, conversation_id="alice-trip-missing")
    print("\n=== NEXT MISSING AFTER UPDATE 2 ===\n", missing_resp)
