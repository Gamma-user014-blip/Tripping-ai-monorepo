...
export async function generatePlans(tripYml: string): Promise<TripPlan[]> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to generate plans: ${response.statusText} - ${errorText}`,
    );
  }
 ...
 */
export async function buildTripRequest(
  vibe: string,
  actions: Array<Array<string | number | null>>,
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(
      `Failed to build trip request: ${response.statusText} - ${errorText}`,
    );
  }
 ...
 */
export async function editPlans(
  plans: TripPlan[],
  userText: string,
): Promise<EditTripPlansResponse> {
  const response = await fetch(`${JSON_AGENT_URL}/api/edit-plans`, {
    ...
    body: JSON.stringify({
      plans,
      user_text: userText,
    } as EditTripPlansRequest),
  });

  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(
      `Failed to edit plans: ${response.statusText} - ${errorText}`,
    );
  }
